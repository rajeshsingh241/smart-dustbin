...
export default function DashboardContent() {
  ...
  useEffect(() => {
    ...
    console.log('🏷️ HTML has dark class?', document.documentElement.classList.contains('dark'));
  ...
  const sendAlert = async (bin: Dustbin) => {
    try {
      console.log('🚨 Mock sending alert for:', bin.id);
      ...
      // Show success
      alert(`Mock alert sent for ${bin.id}! In production, this would notify municipal authorities.`);
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      case 'critical': return 'bg-red-100 text-red-800 border-red-300 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800';
      case 'warning': return 'bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-900/30 dark:text-yellow-300 dark:border-yellow-800';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-400 mt-2">Using mock data - Firebase disabled</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-gray-900 dark:to-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Smart Dustbin Dashboard</h1>
            <p className="text-gray-600 dark:text-gray-300 mt-1">
              Monitor and manage dustbins in real-time
              <span className="text-sm text-amber-600 dark:text-amber-400 ml-2">(Using Mock Data)</span>
            </p>
          </div>
          <button
        ...
        <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <p className="text-amber-800 dark:text-amber-300">
              <strong>Development Mode:</strong> Firebase is temporarily disabled. Using mock data for demonstration.
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border dark:border-gray-700 hover:shadow-md dark:hover:shadow-gray-800/50 transition-shadow">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Bins</p>
                    <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">{dustbins.length}</p>
                  </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border dark:border-gray-700 hover:shadow-md dark:hover:shadow-gray-800/50 transition-shadow">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-red-600 dark:text-red-400 mt-2">{criticalBins}</p>
                  </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border dark:border-gray-700 hover:shadow-md dark:hover:shadow-gray-800/50 transition-shadow">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">{avgFillLevel}%</p>
                  </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 overflow-hidden hover:shadow-md dark:hover:shadow-gray-800/50 transition-shadow">
              <div className="px-6 py-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                <div className="flex justify-between items-center">
                  <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Live Dustbin Status</h2>
                  <span className="text-xs text-gray-500 dark:text-gray-400">(Simulated updates every 5s)</span>
                </div>
              </div>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 dark:bg-gray-900/50 border-b dark:border-gray-700">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Fill Level</th>
                      ...
                      <tr key={bin.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm font-medium text-gray-900 dark:text-white">{bin.id}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900 dark:text-white">{bin.location}</div>
                          <div className="text-xs text-gray-500 dark:text-gray-400">{bin.address}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{bin.zone}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <div className="w-24 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                              <div
                                className={`h-2 rounded-full transition-all ${
                                  bin.fillLevel >= 80 ? 'bg-red-500 dark:bg-red-400' :
                                  bin.fillLevel >= 50 ? 'bg-yellow-500 dark:bg-yellow-400' : 'bg-green-500 dark:bg-green-400'
                                }`}
                                style={{ width: `${bin.fillLevel}%` }}
                              />
                            </div>
                            <span className="text-sm font-medium text-gray-900 dark:text-white">{bin.fillLevel}%</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 hover:shadow-md dark:hover:shadow-gray-800/50 transition-shadow">
            <div className="px-6 py-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400">(Mock Data)</span>
              </div>
            </div>
            <div className="p-6">
              {alerts.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Send alerts from critical bins</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {alerts.map((alert) => (
                    <div key={alert.id} className="flex items-center justify-between p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors">
                      <div className="flex items-center gap-4">
                        <AlertTriangle className="w-5 h-5 text-red-600 dark:text-red-400" />
                        <div>
                          <p className="font-medium text-red-900 dark:text-red-300">{alert.dustbinId}</p>
                          <p className="text-sm text-red-700 dark:text-red-400">{alert.location}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full">
            <div className="flex items-center justify-between p-6 border-b dark:border-gray-700">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Send Alert</h3>
              <button 
            ...
            <div className="p-6">
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4">
                <p className="font-medium text-red-900 dark:text-red-300">Critical Alert (Mock)</p>
                <p className="text-sm text-red-700 dark:text-red-400 mt-1">
                  {selectedBin.id} at {selectedBin.location} - {selectedBin.fillLevel}%
                </p>
                <p className="text-xs text-amber-700 dark:text-amber-400 mt-2">
                  Note: In production, this would send email/SMS to municipal authorities.
                </p>
              </div>
      ...
      {showAddModal && (
        <AddDustbinModal
          ...
          onSuccess={() => {
            ...
            alert('Mock dustbin added successfully! In production, this would save to Firebase.');
